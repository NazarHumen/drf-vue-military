...
def generate_receipt_pdf(order) -> BytesIO:
    ...
    header_data = [
        [
            Paragraph("<b>MILITARY</b>", _style("logo", fontName="DejaVu-Bold", fontSize=22, textColor=DARK)),
            Paragraph(f"<b>#{order.id}</b>", _style("num", fontName="DejaVu-Bold", fontSize=20, textColor=OLIVE, alignment=2)),
        ],
        [
            Paragraph(_("ЧЕК / RECEIPT"), _style("sub", fontSize=9, textColor=GRAY, letterSpacing=3)),
            Paragraph(order.created_timestamp.strftime("%d.%m.%Y"), _style("date", fontSize=9, textColor=GRAY, alignment=2)),
        ],
    ]
    ...
    payment_method = _("При отриманні") if order.payment_on_get else _("Онлайн (Stripe)")
    delivery = (
        order.delivery_address
        if order.requires_delivery and order.delivery_address
        else (_("Потрібна доставка") if order.requires_delivery else _("Самовивіз"))
    )
    customer = order.user.get_full_name() or order.user.username

    info_data = [
        [
            Paragraph(_("Замовлення:"), s_small), Paragraph(f"<b>#{order.id}</b>", s_bold),
            Paragraph(_("Клієнт:"), s_small), Paragraph(f"<b>{customer}</b>", s_bold),
        ],
        [
            Paragraph(_("Дата замовлення:"), s_small),
            Paragraph(f"<b>{order.created_timestamp.strftime('%d.%m.%Y %H:%M')}</b>", s_bold),
            Paragraph("Email:", s_small), Paragraph(f"<b>{order.email}</b>", s_bold),
        ],
        [
            Paragraph(_("Дата оплати:"), s_small), Paragraph(f"<b>{paid_at}</b>", s_bold),
            Paragraph(_("Телефон:"), s_small), Paragraph(f"<b>{order.phone_number}</b>", s_bold),
        ],
        [
            Paragraph(_("Спосіб оплати:"), s_small), Paragraph(f"<b>{payment_method}</b>", s_bold),
            Paragraph(_("Доставка:"), s_small), Paragraph(f"<b>{delivery}</b>", s_bold),
        ],
    ]
    ...
    # ── Items table ───────────────────────────────────────────────────────────
    s_th = _style("th", fontName="DejaVu-Bold", fontSize=9, textColor=colors.white)
    s_th_c = _style("th_c", fontName="DejaVu-Bold", fontSize=9, textColor=colors.white, alignment=1)
    s_th_r = _style("th_r", fontName="DejaVu-Bold", fontSize=9, textColor=colors.white, alignment=2)
    s_td_c = _style("td_c", alignment=1)
    s_td_r = _style("td_r", alignment=2)
    s_total_label = _style("tot_l", fontName="DejaVu-Bold", fontSize=12, alignment=2)
    s_total_val = _style("tot_v", fontName="DejaVu-Bold", fontSize=12, textColor=OLIVE, alignment=2)
    ...
    def price_cell(uah, usd):
        ...
        usd_line = f'<br/><font size="8" color="#CC3333">${usd:.2f}</font>' if usd else ""
    ...
    for item in items:
        unit_usd = item.products_price_usd() / item.quantity if item.quantity else 0
        rows.append([
    ...
    rows.append([
        ...
        Paragraph(
            f"{total} {_('грн.')}<br/><font size='10' color='#CC3333'>${total_usd:.2f}</font>",
            s_total_val,
        ),
    ])
    ...
    # ── Paid stamp ────────────────────────────────────────────────────────────
    stamp_table = Table(
        [[Paragraph(_("✓  ОПЛАЧЕНО"), _style("stamp", fontName="DejaVu-Bold", fontSize=13, textColor=OLIVE, alignment=2))]],
        colWidths=["100%"],
    )
    ...
    # ── Footer ────────────────────────────────────────────────────────────────
    elems.append(HRFlowable(width="100%", thickness=0.5, color=LIGHT_GRAY, spaceAfter=0.2 * cm))
